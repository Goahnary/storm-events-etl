...
//read states in to file
$stateNames = file("states.csv", FILE_IGNORE_NEW_LINES);

$convertedStates = [];

for($i=0; $i < sizeof($stateNames); $i++) {
	$convertedState = convert_state($stateNames[$i], "abbrev");
	$convertedStates[$i] = $convertedState;
}

$resultFile = fopen("stateAbbreviations.csv", "w") or die("unable to open file");

//fwrite($resultFile, "STATE_REGION_CODE");
for($j=0; $j < sizeof($convertedStates); $j++) {
	$line = "US-" . $convertedStates[$j] . "\n";
